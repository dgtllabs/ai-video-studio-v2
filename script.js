...
const Storage = (() => {
  ...
  const DEFAULTS = {
    gemini: { keyName: 'avis_gemini_key', modelName: 'avis_gemini_model', defaultModel: 'gemini-2.5-flash' },
...
const UI = (() => {
  ...
  const PROVIDER_INFO = {
    gemini: {
      label: 'Gemini',
      keyPlaceholder: 'AIza...',
      hint: 'Free tier — no credit card required. Rate-limited, which is plenty for personal use.',
      ...
      models: [
        { value: 'gemini-2.5-flash', label: 'gemini-2.5-flash (recommended — free, vision)' },
        { value: 'gemini-2.5-flash-lite', label: 'gemini-2.5-flash-lite (fastest, highest free quota)' },
...
(() => {
  ...
  function handleApiError(err) {
    ...
    if (msg === 'PARSE_ERROR') { UI.toast('The model returned an unexpected response. Try again.'); return; }
    if (msg.startsWith('API_ERROR:401') || msg.startsWith('API_ERROR:400') || msg.startsWith('API_ERROR:403')) {
      UI.toast(`${providerLabel} rejected that key — check it and try again.`); UI.openModal(); return;
    }
    if (msg.startsWith('API_ERROR:429')) { UI.toast(`Rate limited by ${providerLabel} — wait a moment and try again.`); return; }
    UI.toast(`Something went wrong calling ${providerLabel}. Check the console for details.`);
  }
